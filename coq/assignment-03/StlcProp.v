...
Fixpoint subst (x : id) (s : tm) (t : tm) : tm :=
  ...
  | tabs x' T t'  => tabs x' T (if eq_id_dec x x' then t' else ([x := s] t'))
  | tapp t1 t2    => tapp ([x := s] t1) ([x := s] t2)
  | tnat n        => tnat n
  | tsucc t'      => [x := s] t'
  | tpred t'      => [x := s] t'
  | tmult t1 t2   => tmult ([x := s] t1) ([x := s] t2)
  | tif0 tc t1 t2 => tif0 ([x := s] tc) ([x := s] t1) ([x := s] t2)
  end
...
Inductive step : tm -> tm -> Prop :=
  ...
  | ST_IfSucc : forall n t1 t2,
    tif0 (tnat (S n)) t1 t2 ==> t2
  | ST_If : forall tc tc' t1 t2,
    tc ==> tc' ->
    tif0 tc t1 t2 ==> tif0 tc' t1 t2
...
Tactic Notation "step_cases" tactic(first) ident(c) :=
  ...
  | Case_aux c "ST_Mult1"
  | Case_aux c "ST_Mult2"
  | Case_aux c "ST_IfZero"
  | Case_aux c "ST_IfSucc"
  | Case_aux c "ST_If"
  ].
...
Inductive has_type : context -> tm -> ty -> Prop :=
  ...
  | T_Succ : forall Gamma (t : tm),
    Gamma |- t \in TNat ->
    Gamma |- tsucc t \in TNat
  | T_Pred : forall Gamma (t : tm),
    Gamma |- t \in TNat ->
    Gamma |- tpred t \in TNat
  | T_Mult : forall Gamma (t1 t2 : tm),
    Gamma |- t1 \in TNat ->
    Gamma |- t2 \in TNat ->
    Gamma |- tmult t1 t2 \in TNat
  | T_If : forall tc t2 t3 T Gamma,
    Gamma |- tc \in TNat ->
    Gamma |- t2 \in T ->
    Gamma |- t3 \in T ->
    Gamma |- tif0 tc t2 t3 \in T
...
Lemma canonical_forms_fun : forall t T1 T2,
  empty |- t \in (TArrow T1 T2) ->
  value t ->
  exists (x : id) (t' : tm), t = tabs x T1 t'.
Proof.
  intros t T1 T2.
  intros HT Hval.
  inversion Hval; subst; inversion HT;

  exists x; exists t0; reflexivity. Qed.
...
Proof with eauto.
  ...
  destruct IHHT2...
    (* value t1 /\ value t2 *)
    assert (Ht1 : exists x t, t1 = tabs x T11 t).
      eapply canonical_forms_fun...
    destruct Ht1 as [x [t Habs]]. subst.
    eexists...
  ...
  destruct IHHT...
    (* t = n *)
    destruct (canonical_forms_nat t) as [n]... subst.
    eexists...
  ...
  destruct IHHT...
    (* t = n *)
    destruct (canonical_forms_nat t) as [m]... subst.
    eexists...
  ...
  destruct IHHT2...
    (* t1 = n /\ t2 = m *)
    destruct (canonical_forms_nat t1) as [n]...
    destruct (canonical_forms_nat t2) as [m]...
    subst...
  ...
  destruct IHHT1...
    (* tc = n *)
    destruct (canonical_forms_nat tc)...
    subst. destruct x; eexists...
    (* tc ==> tc' *)
    destruct H. eexists... Qed.
...
Inductive appears_free_in : id -> tm -> Prop :=
  ...
  | afi_mult2 : forall x t1 t2,
    appears_free_in x t2 ->
    appears_free_in x (tmult t1 t2)
  | afi_ifc : forall x tc t1 t2,
    appears_free_in x tc ->
    appears_free_in x (tif0 tc t1 t2)
  | afi_if1 : forall x tc t1 t2,
    appears_free_in x t1 ->
    appears_free_in x (tif0 tc t1 t2)
  | afi_if2 : forall x tc t1 t2,
    appears_free_in x t2 ->
    appears_free_in x (tif0 tc t1 t2).
...
Proof.
  ...
  try (solve [inversion HT; eauto]).

  Case "afi_abs".
  inversion HT. subst.
  apply IHHafi in H5.
  rewrite -> extend_neq in H5;
  assumption. Qed.
...
Proof.
  ...
  unfold closed.
  intros x Contra.
  destruct (free_in_context x t T empty); eauto.
  inversion H. Qed.
...
Proof with eauto.
  ...
  destruct (eq_id_dec x i); subst.
    ...
    eapply context_invariance...
    intros x Contra.
    destruct (free_in_context _ _ T empty Contra) as [T' HT']...
    inversion HT'.

    (* x <> i *)
    apply T_Var. rewrite extend_neq in H1...
...
Proof with eauto.
  ...
  has_type_cases (induction HT) Case;
  intros t' Hstep; subst;

  try (solve [inversion Hstep; subst; auto]).
