...
Fixpoint subst (x:id) (s:tm) (t:tm) : tm :=
  ...
  | tunit => tunit
  | tlet y t1 t2 =>
      (* Assumption: [let] is non-recursive. *)
...
Inductive step : tm -> tm -> Prop :=
  ...
  | ST_FixAbs : forall f T t,
       tfix (tabs f T t) ==> [f := tfix (tabs f T t)] t
...
Tactic Notation "step_cases" tactic(first) ident(c) :=
  ...
  | Case_aux c "ST_LetVal" | Case_aux c "ST_LetStep"
  ...
  | Case_aux c "ST_FixAbs" | Case_aux c "ST_FixStep"
  ].
...
Proof with eauto.
  ...
  Case "T_Let".
    eapply T_Let...
    apply IHhas_type2. intros x' Hafi.
    unfold extend.
...
Proof with eauto.
  ...
  Case "T_Fix".
    inversion HT. subst.
    eapply substitution_preserves_typing... Qed.
