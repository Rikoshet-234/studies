...
Ltac contradiction_value :=
  match goal with
  | Hvalue : value ?V |- _ =>
    apply value__normal in Hvalue;
    contradiction Hvalue;
    eauto;
    fail
  end.
...
Proof with eauto.
  ...
  (* Now, tactic magic. *)
  try contradiction_value;
  try step_inversion. Qed.
...
Proof.
  ...
  intros t t';
  intros HT Hstep HR;
  destruct HR as [typable_empty_t [halts_t RRt]].
  ...
  (* This should be solvable with `eauto using step_preserves_halting` but somehow
     it is not. *)
...
Proof with eauto.
  induction t;
  intros x H t';
  unfold subst; fold subst; eauto;
  try (rewrite -> IHt1; eauto; rewrite -> IHt2; eauto).
...
Tactic Notation "prove_msubst" :=
  intros ss;  
  induction ss as [| a ss' IHss']; intros;
  try reflexivity;
  destruct a; simpl; rewrite <- IHss'; auto.

Lemma msubst_pair : forall ss t1 t2,
  msubst ss (tpair t1 t2) = tpair (msubst ss t1) (msubst ss t2).
Proof. prove_msubst. Qed.
...
Lemma msubst_if : forall ss tc t1 t2,
  msubst ss (tif tc t1 t2) = tif (msubst ss tc) (msubst ss t1) (msubst ss t2).
Proof. prove_msubst. Qed.
...
Proof. intros t1 t1' t2 STM. induction STM; eauto. Qed.

Lemma multistep_Pair2 : forall v t t',
  value v -> (t ==>* t') -> (tpair v t) ==>* (tpair v t').
Proof. intros v t t' V STM. induction STM; eauto. Qed.

Lemma multistep_Pair : forall t1 v1' t2 t2',
  value v1' ->
  t1 ==>* v1' -> t2 ==>* t2' -> tpair t1 t2 ==>* tpair v1' t2'.
Proof.
  intros t1 v1' t2 t2' Hv Ht1 Ht2.
  apply multi_trans with (y := tpair v1' t2).
  apply multistep_Pair1; auto.
  apply multistep_Pair2; auto. Qed.

Lemma multistep_Fst : forall t t',
  t ==>* t' -> tfst t ==>* tfst t'.
Proof. intros t t' STM. induction STM; eauto. Qed.

Lemma multistep_Snd : forall t t',
  t ==>* t' -> tsnd t ==>* tsnd t'.
Proof. intros t t' STM. induction STM; eauto. Qed.

Lemma multistep_IfTrue' : forall tc v,
  tc ==>* v -> v = ttrue -> forall t1 t2, tif tc t1 t2 ==>* t1.
Proof. intros tc v STM HV. induction STM; subst; eauto. Qed.

(* I couldn't prove following lemma directly (weird induction behaviour), so I
   used the one above as a workaround. This applies also to similiar
   [multistep_IfFalse]. *)
 
Lemma multistep_IfTrue : forall tc t1 t2,
  tc ==>* ttrue -> tif tc t1 t2 ==>* t1.
Proof. eauto using multistep_IfTrue'. Qed.

Lemma multistep_IfFalse' : forall tc v,
  tc ==>* v -> v = tfalse -> forall t1 t2, tif tc t1 t2 ==>* t2.
Proof. intros tc v STM HV. induction STM; subst; eauto. Qed.

Lemma multistep_IfFalse : forall tc t1 t2,
  tc ==>* tfalse -> tif tc t1 t2 ==>* t2.
Proof. eauto using multistep_IfFalse'. Qed.

Lemma multistep_bool : forall t v',
  has_type \empty t TBool -> t ==>* v' -> value v' -> v' = ttrue \/ v' = tfalse.
Proof.
  intros t v'.
  intros HT Hmstep HV.
  induction Hmstep.
  inversion HV; subst; try (inversion HT); auto.
  eauto using preservation. Qed.
...
Proof. 
  ...
  (* In the following code I use tactic notations quite extensively to avoid
     a lot of code duplication. Perhaps this is an abuse of this mechanism
     but I could not find any better macro-like solution to do what I want. *)

  Case "T_Pair".
    rewrite msubst_pair.
    pose proof (IHHT1 c H env0 V) as P1.
    pose proof (IHHT2 c H env0 V) as P2.
    destruct (R_halts P1) as [t1']. destruct H0.
    destruct (R_halts P2) as [t2']. destruct H2.
    unfold R. fold R.
    repeat split; eauto.

    (* has type *)
    Tactic Notation "solve_type" ident(V) :=
      eapply msubst_preserves_typing;
      [ apply V
      | eapply context_invariance; [eauto | 
        intros; rewrite <- mextend_lookup; auto] ].

    eapply T_Pair; solve_type V.

    (* halts *)
    exists (tpair t1' t2').
    split.
    apply multistep_Pair; auto.
    apply v_pair; auto.

    (* predicate *)
    Tactic Notation "solve_R" ident(t) ident(t1) ident(t2) ident(P)
                              reference(tcons) reference(cast) reference(step) :=
      apply multistep_preserves_R' with (t' := t);
      [ eapply tcons; apply T_Pair; eauto using R_typable_empty
      | apply multi_trans with (y := cast (tpair t1 t2));
        [ apply step; apply multistep_Pair; eauto | eauto ]
      | apply multistep_preserves_R with (t' := t) in P; auto].

    solve_R t1' t1' t2' P1 T_Fst tfst multistep_Fst.
    solve_R t2' t1' t2' P2 T_Snd tsnd multistep_Snd.

  Tactic Notation "solve_cast" reference(msubst_cast)
                               constr(H)
                               simple_intropattern(HRT) :=
    rewrite msubst_cast;
    pose proof H as P;
    unfold R in P; fold R in P;
    destruct P as [_ [_ HRT]];
    assumption.

  Case "T_Fst". solve_cast msubst_fst (IHHT c H env0 V) [HRT1 _].
  Case "T_Snd". solve_cast msubst_snd (IHHT c H env0 V) [_ HRT2].

  Tactic Notation "simpl_bool" :=
    simpl; rewrite msubst_closed; repeat split; eauto;
    try (intros x Contra; inversion Contra).

  Case "T_True". simpl_bool; exists ttrue; split; eauto.
  Case "T_False". simpl_bool; exists tfalse; split; eauto.

  Case "T_If".
    rewrite msubst_if.
    pose proof (IHHT1 c H env0 V) as PC.
    pose proof (IHHT2 c H env0 V) as PTrue.
    pose proof (IHHT3 c H env0 V) as PFalse.
    destruct PC as [HCtype [HChalts _]].
    destruct HChalts as [b' [Hsb Hvb]].
    destruct (multistep_bool (msubst env0 t0) b'); auto.

    Tactic Notation "solve_if_branch" ident(env) ident(t) reference(mstep) :=
    apply multistep_preserves_R' with (t' := msubst env t);
    eauto using R_typable_empty;
    apply mstep; subst; assumption.

    solve_if_branch env0 t1 multistep_IfTrue.
    solve_if_branch env0 t2 multistep_IfFalse. Qed.
