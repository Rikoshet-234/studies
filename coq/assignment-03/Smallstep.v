...
(** Contrary to what the exercise tells, [compile] was not given in the [Imp]
    chapter. Instead, it was left as an exercise. Therefore, I implement it
    here.

    To avoid notation clashes with next chapters, I wrap everything in
    additional module. *)

Module SosMachine.

Fixpoint compile (a : aexp) : prog :=
  match a with
  | ANum n       => [SPush n]
  | AId x        => [SLoad x]
  | APlus a1 a2  => compile a1 ++ compile a2 ++ [SPlus]
  | AMinus a1 a2 => compile a1 ++ compile a2 ++ [SMinus] 
  | AMult a1 a2  => compile a1 ++ compile a2 ++ [SMult]
  end.

Notation "a '/' st '==>a*' a'" := (multi (astep st) a a')
  (at level 40, st at level 39, a' at level 39).

Notation "c '/' st '==>s*' c'" := (stack_multistep st c c')
  (at level 40, st at level 39, c' at level 39).

(** Since I am not completely sure whether my understanding of correctness
    with respect to small step semantics of aexp is right I first implement
    original "correctness" definition as stated in the exercise contents.
    This definition is based on [s_compile_correct] defined in  the [Imp]
    chapter. *)

Definition compiler_is_correct_statement : Prop :=
  forall (st : state) (a : aexp),
  (compile a, []) / st ==>s* ([], [aeval st a]).

Lemma concat_singleton : forall (T : Type) (x : T) (xs : list T),
  [x] ++ xs = x :: xs.
Proof. reflexivity. Qed.

Tactic Notation "prove_correct_binary_induction" ident(IHleft) ident(IHright) :=
...
Hint Constructors stack_step.
Hint Constructors multi.

Lemma compiler_is_correct_general :
  forall (st : state) (a : aexp) (p : prog) (s : stack),
  (compile a ++ p, s) / st ==>s* (p, aeval st a :: s).
Proof with eauto.
  intros st a.
  ...
  intros p s;
  simpl;
  try (prove_correct_binary_induction IHa1 IHa2)...

  (* Unfortunately, I was not able to use ';' to get rid of these three. *)
  rewrite -> concat_singleton...
  rewrite -> concat_singleton...
  rewrite -> concat_singleton... Qed.

Theorem compiler_is_correct : compiler_is_correct_statement.
Proof.
  intros st a.
  assert (app_nil : compile a = compile a ++ []).
    eauto using app_nil_r.
  rewrite -> app_nil.
  apply compiler_is_correct_general. Qed.

(** Now, the part about correctness with respect to small step semantics.
    My definition works like this: if one expression evaluates to other,
    then compiled programs for these expressions should yield the same results
    after execution. *)

Definition compiler_is_correct_sos_statement : Prop :=
  forall (a a' : aexp) (st : state), a / st ==>a* a' ->
  exists (s : stack),
  (compile a, []) / st ==>s* ([], s) /\ (compile a', []) / st ==>s* ([], s).

Lemma step_aeval : forall (a a' : aexp) (st : state),
  a / st ==>a a' ->
  aeval st a = aeval st a'.
Proof.
  intros a a' st Hstep.
  induction Hstep; subst; simpl; eauto. Qed.

Lemma multistep_aeval : forall (a a' : aexp) (st : state),
  a / st ==>a* a' ->
  aeval st a = aeval st a'.
Proof.
  intros a a' st Hstep.
  induction Hstep.
  reflexivity.
  apply step_aeval in H.
  rewrite -> H.
  rewrite -> IHHstep.
  reflexivity. Qed.

Theorem compile_is_correct_sos : compiler_is_correct_sos_statement.
Proof.
  intros a a' st Hmstep.
  eexists.
  split.
  apply compiler_is_correct.
  erewrite -> multistep_aeval.
  apply compiler_is_correct.
  assumption. Qed.
